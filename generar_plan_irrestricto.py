...
ws_parametros = wb_parametros['Tipo de venta']
dict_tipo_venta = {"Venta Directa": [], "Venta Local": []}
tipo_venta = "Venta Directa"

for row in ws_parametros.iter_rows(min_row = 2, max_row = ws_parametros.max_row, values_only = True):
  if row[1] is None:
    break
...
dict_otro_tipo = {}
i = 2
for row in ws_produccion.iter_rows(min_row = 6, max_row = ws_produccion.max_row, values_only = True):
  if row[22] is None:
    break
  llave = row[22]
  sku = row[23]
  ...
  if oficina.lower() in dict_tipo_venta[seleccion_tipo_venta]:
    ...
    if str(sku) in dict_materiales.keys():
      ws.cell(row = i, column = 2).value = dict_materiales[str(sku)]['sector']
      ws.cell(row = i, column = 5).value = dict_materiales[str(sku)]['descripcion'] # E:Descripción
      ws.cell(row = i, column = 6).value = dict_materiales[str(sku)]['nivel_2']     # F: 
      ws.cell(row = i, column = 7).value = dict_materiales[str(sku)]['nivel_3']     # G:
    
    else:
      if 'PV' or 'Pavo' in descripcion:
        ws.cell(row = i, column = 2).value = PV      
      elif 'PO' or 'Pollo' in descripcion:
        ws.cell(row = i, column = 2).value = PO                              
      elif 'GO' or 'Cerdo' in descripcion:
        ws.cell(row = i, column = 2).value = GO    
      elif 'GA' or 'Gallina' in descripcion:
        ws.cell(row = i, column = 2).value = GA
      else:
        ws.cell(row = i, column = 2).value = 'Elaborado' # Sector = 'Elaborado'
    i += 1
...
print("--- %s Despues de producción ---" % (time.time() - start_time))
################## RV Venta mes n+1 ##################
...
for i, row in enumerate(ws_irrestricto.iter_rows(min_row = 2, max_row = ws_irrestricto.max_row, values_only=True), start = 2):
  ...
  if llave_actual.lower() in dict_venta_n1:
    ...
    ws_irrestricto.cell(row = i, column = 9).value = dict_venta_n1[llave_actual.lower()]
...
for key, value in dict_venta_no_asignadas.items():
  sku = value['sku']
  if str(sku) in dict_materiales.keys():
    ws_irrestricto.cell(row = i, column = 2).value = dict_materiales[str(sku)]['sector']
    ws_irrestricto.cell(row = i, column = 5).value = dict_materiales[str(sku)]['descripcion'] # E:Descripción
    ws_irrestricto.cell(row = i, column = 6).value = dict_materiales[str(sku)]['nivel_2']     # F: 
    ws_irrestricto.cell(row = i, column = 7).value = dict_materiales[str(sku)]['nivel_3']     # G:

  ws_irrestricto.cell(row = i, column = 1).value = key
  ws_irrestricto.cell(row = i, column = 3).value = value['office']
  ws_irrestricto.cell(row = i, column = 4).value = sku
  ws_irrestricto.cell(row = i, column = 5).value = value['description']
  ws_irrestricto.cell(row = i, column = 9).value = value['monthly_sales']
...
for row in ws_por_producir.iter_rows(min_row = 4, max_row = max_row_por_producir, values_only=True):
  key = row[8] # código SAP
  value = row[9] # valor 
...
dict_por_despachar = {}
for row in ws_por_despachar.iter_rows(min_row = 8, max_row = ws_por_despachar.max_row, values_only=True):
  if row[1] is None:
    break
  llave = row[1].lower()
  kg_despachar = row[4]
...
for row in range(2, ws_irrestricto.max_row + 1):
  ...
  if sku_actual in dict_stock:
    ws_irrestricto.cell(row = row, column = 12).value = dict_stock[sku_actual]
  elif sku_actual not in dict_stock:
  ...
  if sku_actual in dict_producir:
    ...
    ws_irrestricto.cell(row = row, column = 13).value = (float(dict_producir[sku_actual])* ponderacion)
...
for i, row in enumerate(ws_irrestricto.iter_rows(min_row = 2, max_row = max_row, values_only=True), start = 2):
  ...
  ws_irrestricto.cell(row = i, column = 21).value = f'=O{i}-T{i}'
...
messageBox(dict_datos_modificados, dict_otro_tipo)
